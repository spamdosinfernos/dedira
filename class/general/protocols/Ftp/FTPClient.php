...
class FTPClient implements IFileSender{
	...
	private function criarSubFilePathDestiny($directoryPathDestiny, $nameDestiny){
		...
		foreach ($arrLetrasDoCaminho as $key => $letra) {
			...
			$directoryPathDestiny .= Configuration::getDirectorySeparatorRemote() . $letra;
	...
	public function upload($filePathOrign, $directoryPathDestiny, $nameDestiny, $callBackFunction = null){
		...
		if(!$this->testingConnection){
			...
			$filePathDestiny .= Configuration::getDirectorySeparatorRemote() . $nameDestiny;
		}else{
			$filePathDestiny = $directoryPathDestiny . Configuration::getDirectorySeparatorRemote() . $nameDestiny;
		}
	...
	public function selftest(){
		...
		$retorno = $this->upload("." . DIRECTORY_SEPARATOR . $nomeDoArquivoDeTeste, Configuration::getLocalRemotoParaEnvioDosArquivos(), $nomeDoArquivoDeTeste);
		if($retorno){
