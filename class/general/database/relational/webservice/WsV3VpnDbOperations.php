...
/**
 * Operações do banco de dados via webservice do servidor V
 */
class WsV3VpnDbOperations implements IDataBaseOperations{
	...
	public function openConnection(){
		$this->wsClient = new SoapClient(Configuration::CONST_WS_V3_VPN_DATABASE_URL, array("trace" => 0, "exceptions" => 1, "user_agent"=>""));
	}
	...
	public function execQuery($query){
		try{
			...
			$return = $this->wsClient->executeProcedure(Configuration::CONST_WS_V3_VPN_DBOPERATIONS_CLASS, $query);
			...
			if(count($return->linhas) == 1){
				$arrCampos[$return->linhas->campos[0]->nome] = $return->linhas->campos[0]->valor;
				$arrCampos[$return->linhas->campos[1]->nome] = $return->linhas->campos[1]->valor;
				$arrResult[] = $arrCampos;
	...
	public function getProcedureCallToken(){
		return Configuration::CONST_WS_V3_VPN_DATABASE_PROCEDURE_CALL_TOKEN;
	}
}
?>