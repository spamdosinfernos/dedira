...
class StorableObject {
	 ...
	 * 
	 * @example $obj = new StorableObject();
	...
	public function load() {
		...
		if (! $this->dataBaseOperator->loadDocument ( $this->id ))
			return null;
	...
	public function erase() {
		if ($this->id == "")
			throw new SystemException ( "Falha ao apagar informação: O evento não tem uma identificação." );
		if ($this->rev == "")
			throw new SystemException ( "Falha ao apagar informação: O evento não tem um número de revisão.", __CLASS__ . __LINE__ );
	...
	private function openDataBaseConection() {
		if (Configuration::CONST_DB_NAME == "")
			throw new SystemException ( "text - Para conectar na base de dados é necessário informar seu nome com 'setDataBaseName()'", __CLASS__ . __LINE__ );
	...
	private function getInformationType($information) {
		...
		if (is_bool ( $information ))
			return self::CONST_INFORMATION_TYPE_BOOLEAN;
		
		if (is_array ( $information ))
			return self::CONST_INFORMATION_TYPE_ARRAY;
		
		if (isset ( $information->CLASSNAME )) {
			if ($information->CLASSNAME != "stdClass")
	...
	private function getVisibility($information) {
		...
		try {
			...
			if ($reflect->hasProperty ( ReflectionProperty::IS_PROTECTED ))
				return ReflectionProperty::IS_PROTECTED;
			if ($reflect->hasProperty ( ReflectionProperty::IS_PRIVATE ))
				return ReflectionProperty::IS_PRIVATE;
			if ($reflect->hasProperty ( ReflectionProperty::IS_PUBLIC ))
	...
	private function generateSerializedProperty($tipo, $property, $visibility) {
		if ($visibility == ReflectionProperty::IS_PROTECTED)
			return self::CONST_FLAG_PROTECTED_PROPERTY . self::CONST_MIDDLE_FLAG_PROTECTED_PROPERTY . self::CONST_FLAG_PROTECTED_PROPERTY . $property;
		if ($visibility == ReflectionProperty::IS_PRIVATE)
			return self::CONST_FLAG_PRIVATE_PROPERTY . get_class ( $this ) . self::CONST_FLAG_PRIVATE_PROPERTY . $property;
		return $property;
	}
	 ...
	 * 
	 * @param string $property
	 *        	- Nome da propriedade tratada (null para objeto raiz)
	 * @param mixed $information
	 *        	- Objeto, string, número, array que contêm a informação
	 */
	...
	private function setDataBaseName($dataBaseName) {
		if (! is_string ( $dataBaseName ))
			throw new SystemException ( "text - O nome da base de dados tem que ser uma string", __CLASS__ . __LINE__ );
	...
	private function convertToArray($info) {
		...
		if (is_object ( $info )) {
			// The saved itens are properties
			$reflection = new ReflectionObject ( $info );
			
			$arrPropriedades = $reflection->getProperties ( ReflectionProperty::IS_PUBLIC | ReflectionProperty::IS_PROTECTED | ReflectionProperty::IS_PRIVATE );
		} else {
			// Or arrays
		...
		if (is_object ( $info ))
			$arrSerial ["CLASSNAME"] = get_class ( $info );
