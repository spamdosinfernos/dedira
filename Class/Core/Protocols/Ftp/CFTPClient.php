<?php
require_once __DIR__ . '/../IFileSender.php';

class FTPClient implements IFileSender{
	...
	private function criarSubFilePathDestiny($directoryPathDestiny, $nameDestiny){
		...
		foreach ($arrLetrasDoCaminho as $key => $letra) {
			...
			$directoryPathDestiny .= CConfiguration::getDirectorySeparatorRemote() . $letra;
	...
	public function upload($filePathOrign, $directoryPathDestiny, $nameDestiny, $callBackFunction = null){
		...
		if(!$this->testingConnection){
			...
			$filePathDestiny .= CConfiguration::getDirectorySeparatorRemote() . $nameDestiny;
		}else{
			$filePathDestiny = $directoryPathDestiny . CConfiguration::getDirectorySeparatorRemote() . $nameDestiny;
		}
	...
	public function selftest(){
		...
		$retorno = $this->upload("." . DIRECTORY_SEPARATOR . $nomeDoArquivoDeTeste, CConfiguration::getLocalRemotoParaEnvioDosArquivos(), $nomeDoArquivoDeTeste);
		if($retorno){
